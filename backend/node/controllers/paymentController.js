...
class PaymentController {
  // Rider initiates payment after ride completed
  async initiateRidePayment(req, res) {
    try {
      const { ride_id } = req.params;
      const { mode, walletPin } = req.body;

      if (!["cash", "upi", "wallet"].includes(mode)) {
        return errorResponse(res, "Invalid payment mode", 400);
      }

      const paymentResult = await paymentService.initiateRidePayment(
        Number(ride_id),
        mode,
        walletPin, // required if mode === wallet
        req.user.user_id
      );

      return successResponse(res, "Payment processed", paymentResult);
    } catch (err) {
      return errorResponse(res, err, err.statusCode || 500);
    }
  }

  // Driver confirms payment after ride (for cash/upi)
  async confirmRidePayment(req, res) {
    try {
      const { ride_id } = req.params;
      const driver_id = req.user.user_id;

      const confirmationResult = await paymentService.confirmPaymentByDriver(
        Number(ride_id),
        driver_id
      );

      return successResponse(res, "Payment confirmed by driver", confirmationResult);
    } catch (err) {
      return errorResponse(res, err, err.statusCode || 500);
    }
  }
